...
const getById = (req, res) => {
    ...
    if (req.pre.preVal === null)
        throw Boom.notFound('The product you are searching for, does nt exists');
...
const post = (req, res) => {
    ...
    console.log(req.pre.preVal);

    if (req.pre.preVal.length > 0)
        throw Boom.badRequest('No puede agregar un producto ya existente');
    ...
    return Object.assign({ 'code': 200, 'message': 'Product Successfully Created', '_id': product._id });
...
const put = (req, res) => {
    ...
    const id              = req.payload.product.id;
...
const deleteFn = (req, res) => {
    ...
    Product.findOneAndRemove({ '_id': productId }, (err) => {
